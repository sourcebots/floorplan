...
def embed_svg(embedded, root, ns, template_dir, team_names=None):
    ...
    # set x, y, height & width from the placeholder
...
def main():
    ...
    parser.add_argument(
        '--specs', type=Path, nargs=argparse.OPTIONAL, default=Path('layouts/'),
        help=(
            "Folder or file containing the JSON that defines what output SVG's are "
    ...
    parser.add_argument('--teams', type=Path, default='team_names.json', help=(
        "The JSON file containing a mapping of number to TLA, defaults to %(default)s"
    ))
    ...
    if args.specs.is_file():
        generate_svg(
            args.specs, args.templates, args.output, args.base_scale, teams_file=args.teams)
        ...
        for spec in args.specs.iterdir():
            ...
            generate_svg(
                spec, args.templates, args.output, args.base_scale, teams_file=args.teams)
    else:
